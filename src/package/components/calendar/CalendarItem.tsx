...
import { requestAnimationFrame, TaroElement } from '@tarojs/runtime'
...
const calendar = {
  ...
  start: '开始',
  monthTitle: (year: number | string, month: number | string) => `${year}年${month}月`,
  today: '今天',
  loadPreviousMonth: '加载上一个月',
  noEarlierMonth: '没有更早月份',
}

const CalendarItem: FC<Partial<CalendarItemProps>> = (props) => {
  const {
    type = 'one',
  ...
  const monthsPanel = useRef<TaroElement>(null)

  const classes = classNames(
    {
      [`ygp-calendar-tile`]: !poppable,
      [`ygp-calendar-nofooter`]: !!isAutoBackFill,
    },
    'ygp-calendar',
  )

  const headerClasses = classNames({
    [`ygp-calendar-header`]: true,
    [`ygp-calendar-header-tile`]: !poppable,
  })
  ...
  const getMonth = (curData: string[], type: string) => {
    ...
    const monthInfo: MonthInfo = {
      curData,
      title: calendar.monthTitle(title.year, title.month),
      monthData: [
      ...
      throttle(async () => {
        if (weeksPanel?.current && monthsPanel?.current) {
          ...
          await new Promise<void>((resolve) => {
            query.select('.calendar-weeks').boundingClientRect()
            query.selectAll('.calendar-month').boundingClientRect()
            query.exec((res) => {
              weeksPanelRes = res[0]
              monthsRes = res[1]
              resolve()
          ...
          for (let i = 0; i < monthsRes.length; i++) {
            if (monthsRes[i]?.top <= top && monthsRes[i]?.bottom >= top) {
  ...
  const loadScroll = () => {
    ...
    requestAnimationFrame(setMonthTitle)
  }
  ...
  // 计算滚动方向和距离
  const setMove = async (move: number, type?: string, time?: number) => {
    ...
    // 获取元素位置
    await new Promise<void>((resolve) => {
      query.select('.ygp-calendar-content').boundingClientRect()
      query.select('.calendar-months-panel').boundingClientRect()
      query.exec((res) => {
        h = res[0].height || 0
        offsetHeight = res[1].height || 0
        resolve()
      })
    })
  ...
  const onTouchEnd = async (event: ITouchEvent) => {
    event.stopPropagation()
    const changedTouches = event.changedTouches[0]
    state.touchParams.lastY = changedTouches.pageY
    state.touchParams.lastTime = event.timeStamp || Date.now()
    ...
    <>
      <View className={classes}>
        ...
        <View className={headerClasses}>
          {poppable ? (
            <>
              <View className='calendar-curr-month'>{yearMonthTitle}</View>
            </>
          ) : (
            ''
          )}
          <View className='calendar-weeks' ref={weeksPanel}>
            {weeks.map((item: string) => (
              <View className='calendar-week-item' key={item}>
                {item}
              </View>
            ))}
          </View>
        ...
        {/* content */}
        <View
          className='ygp-calendar-content'
          onTouchStart={onTouchStart}
          onTouchMove={onTouchMove}
          onTouchEnd={onTouchEnd}
          ref={months}
        >
          <View className='calendar-months-panel' ref={monthsPanel}>
            <View className='calendar-loading-tip'>
              {!unLoadPrev ? calendar.loadPreviousMonth : calendar.noEarlierMonth}
            </View>
            {monthsData.map((month: any, key: number) => (
              <View className='calendar-month' key={key}>
                <View className='calendar-month-title'>{month.title}</View>
                <View className='calendar-month-con'>
                  <View className={monthitemclasses}>
                    {month.monthData &&
                      month.monthData.map((day: Day, index: number) => (
                        ...
                        >
                          <View className='calendar-day'>{day.type === 'curr' ? day.day : ''}</View>
                          {isCurrDay(month, day.day) ? (
                            <View className='calendar-curr-tips'>{calendar.today}</View>
                          ) : (
                            ''
                          )}
                          {isStartTip(day, month) ? (
                            <View className='calendar-day-tip'>{calendar.start}</View>
                          ) : (
                            ''
                          )}
                          {isEndTip(day, month) ? (
                            <View className='calendar-day-tip'>{calendar.end}</View>
                          ) : (
                            ''
                          )}
                        </View>
